  ...
  for (i = 0, l = list.length; i < l; i++) {
    ...
    if (dirRE.test(name)) {
      ...
      if (bindRE.test(name)) { // v-bind
        ...
        addHandler(el, name, value, modifiers, false, warn$3);
...
var styleValueToCssMap = function (styleValue) {
  var cssMap = {};
  try {
...
function getUpdateStyles (oldVnode, vnode) {
   /**
  ...
  var elm = vnode.elm;
  var oldStyle = oldVnode && oldVnode.data ? oldVnode.data.style : null;
  var baseStyle;
